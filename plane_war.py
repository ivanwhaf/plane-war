...
def main():
    ...
    size = (480, 852)  # 屏幕大小 width height
    ...
    while run:
        ...
        # 绘制按钮
        pg.draw.rect(screen, (195, 200, 201), (430, 800, 45, 45), 0)
        # if not pause:
        ...
        if hero.hp > 0:
            ...
            # 空格键为按下状态
            accumulative_fire += 1
            if key_pressed[pg.K_SPACE] and accumulative_fire > FPS / FIRE_RATE:
                accumulative_fire = 0
                # 发射子弹0
        ...
        for bullet in bullets:
            ...
            if isinstance(bullet, Bullet0):
                ...
                if hero.hp > 0:
                    for enemy in enemies:
                        if enemy.x - enemy.w / 2 <= bullet.x <= enemy.x + enemy.w / 2:
                            if enemy.y - enemy.h / 2 <= bullet.y <= enemy.y + enemy.h / 2:
                                if enemy.hp > 0:
            ...
            # 敌人发出的类型1子弹
                ...
                # 碰撞检测
                if hero.hp > 0:
                    if hero.x - hero.w / 2 <= bullet.x <= hero.x + hero.w / 2:
                        if hero.y - hero.h / 2 <= bullet.y <= hero.y + hero.h / 2:
        ...
        for bomb in bombs:
            ...
            if isinstance(bomb, Bomb0):
                # 碰撞检测
                if hero.hp > 0:
                    if hero.x - hero.w / 2 <= bomb.x <= hero.x + hero.w / 2:
                        if hero.y - hero.h / 2 <= bomb.y <= hero.y + hero.h / 2:
                            bombs.remove(bomb)
                            print('get bomb0!')
                            hero.bomb0_count += 30
                            continue
        ...
        f1rect = f1.render_to(screen, (0, 832), "Score: " + str(score),
                              fgcolor=(0, 0, 0), bgcolor=(195, 200, 201))
